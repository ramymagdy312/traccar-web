...
const ActivityDetailsReportPage = () => {
  ...
  const [columns, setColumns] = usePersistedState('activityDetailsColumns', columnsArray.map(([key]) => key));
  ...
  const handleSubmit = useCatch(async ({ deviceIds, groupIds, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/api/reports/activity/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/api/reports/activity/mail?${query.toString()}`);
      if (!response.ok) {
        throw Error(await response.text());
      }
    } else {
      setLoading(true);
      try {
        const response = await fetch(`/api/reports/activity?${query.toString()}`, {
...
export default ActivityDetailsReportPage;
